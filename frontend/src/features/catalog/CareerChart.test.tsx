...
describe("CareerChart", () => {
  ...
  it("marks the best season, not merely the highest number", () => {
    ...
    render(<CareerChart seasons={[pitching(2019, 2.33), pitching(2026, 5.65)]} accent="#f00" />);
  ...
  it("offers only the metrics that belong to the stat group", () => {
    render(<CareerChart seasons={[pitching(2025, 3.1), pitching(2026, 4.2)]} accent="#f00" />);
  ...
  it("says so when the chosen metric was never ingested", () => {
    ...
    render(<CareerChart seasons={seasons} accent="#f00" />);
    expect(screen.getByText(/not .*recorded across enough seasons|No wOBA recorded/i)).toBeInTheDocument();
  });

  it("names each point for a reader who hovers", () => {
    render(<CareerChart seasons={[hitting(2025, 0.36), hitting(2026, 0.39)]} accent="#f00" />);
    const chart = screen.getByRole("img");
